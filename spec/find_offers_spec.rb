...
RSpec.describe FindOffers do
  ...
  context "opening offer pages" do
    ...
    it "creates url of offer page" do
      ...
      expect(url).to eq("https://www.discogs.com/sell/list?master_id=26600&limit=100&page=1")
    end
    ...
    context "all offers found" do
      ...
      it "returns urls for all pages" do
        ...
        urls_regex = /https:\/\/www.discogs.com\/sell\/list\?master_id=26600&limit=100&page=\d/
  ...
  context "info about the offer" do
    ...
    it "offer id" do
      @offers.each do |offer|
        expect(offer["id"]).to match /\d{8,9}/
      end
    end
    ...
    context "condition" do
      ...
      it "media" do
        pending "fails at non-existing id"
        @offers.each_with_index do |offer, i|
          expect(conditions).to include(offer["media_condition"]),
                                "Expected: one of #{conditions}},
                                        Got :#{@offers[i]["media_condition"]}, failed at offer having id #{@offers[i]["id"]}"
        end
        fail
      end
    ...
    it "price in euro" do
      @offers.each do |offer|
        expect(offer["price_euro"]).to match /\d{1,3}\.\d{2}/
      end
    end

    it "release year" do
      pending "get info through API"
      fail

      @offers.each do |offer|
        expect(offer["release_year"]).to match /\d{4}/
      end
    end
